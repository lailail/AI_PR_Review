...
历史记录用于体现“上下文理解”的本地积累方式：系统不仅能理解当前 PR 的标题、描述、changed files 和 patch，也能知道当前浏览器之前分析过同一仓库的哪些 PR。当前阶段只展示历史记录和摘要，不会把历史 PR 一次性提交给模型做跨 PR 对比。

为控制体积和降低敏感代码暴露风险，历史记录不会保存完整 diff，也不会保存完整仓库代码。系统只保存 AI 总结、风险数量、Review 建议数量，以及截断后的 `patchDigest`。清空浏览器缓存或点击页面中的“清空历史”后，这些记录会被删除。
...
历史记录不保存完整 diff，也不保存完整仓库代码。它的作用是提供“之前这个仓库发生过什么”的摘要信息，而不是替代当前 PR 的真实 diff。
