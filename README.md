...
当前仓库处于 PR 3 阶段：DeepSeek 生成 PR 变更总结。
...
PR 变更总结由服务端调用 DeepSeek Chat Completions 接口生成。
...
前端不会读取 DeepSeek API Key，只会调用本项目的 `/api/ai-summary` 后端接口。
...
5. 系统调用 DeepSeek 生成结构化 PR 总结，后续继续扩展风险识别和 Review 建议。
...
当前 PR 3 阶段会将 PR 标题、描述、变更规模、文件列表和截断后的 patch 发送给 DeepSeek，要求模型返回 JSON 格式的结构化总结。
